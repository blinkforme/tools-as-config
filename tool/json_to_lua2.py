...
def dic_to_lua_str(data, layer=0, field_meta={}):
    ...
    if isinstance(data, string_types):
        ...
        yield ('}')
    elif d_type is dict or d_type is OrderedDict:
        yield ("\n")
        yield (space_str(layer))
        yield ("{\n")
        data_len = len(data)
...
def render_lua_unity(data_dic, sheet_meta):
    field_meta = sheet_meta.get('field_meta', {})
    ...
    for key in data_dic:
        ...
        for it in dic_to_lua_str(data_dic[key], 0, field_meta):
            bytes += it
        data_dic[key] = bytes
    tpl = env.get_template('lua_unity_template.html')

    return tpl.render(data_dic=data_dic, is_str=is_str, sheet_name=sheet_meta["sheet_name"])


def render_lua(data_dic, sheet_meta):
    field_meta = sheet_meta.get('field_meta', {})
    ...
    for key in data_dic:
        ...
        for it in dic_to_lua_str(data_dic[key], 0, field_meta):
            bytes += it
        data_dic[key] = bytes
    tpl = env.get_template('lua_server_template.html')
...
def render_unity_manifest(jsonStr):
    tpl = env.get_template('manifest_unity.html')
    return tpl.render(content=jsonStr)
