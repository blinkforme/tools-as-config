...
from json_to_ts import str_to_as3_class, render_manifest_ts
from json_to_lua import render_lua, render_py
...
def process_sheet_metadata(sh):
    ...
    field_meta[pk_name] = {
        ...
        'as3_type': type_to_ts_type(field_type)
    }
...
def process_xlsx(file_path):
    ...
    for s in wb.sheets():
        ...
        if not is_server_only:
            # 导出AS3配表对象
            for path in AS3_CLAZZ_TARGET_PATHS:
                as3_file_path = os.path.join(path, "{file_name}.ts".format(file_name=sheet_name))
                if IS_LANG_TEMPLATE:
                    write_file(str_to_ts_class_lang(confusion_meta,CONFIG_MANAGER_PATH), as3_file_path)
                else:
...
def entry():
    ...
    if AS3_CLAZZ_TARGET_PATHS:
        for path in AS3_CLAZZ_TARGET_PATHS:
            write_file(render_manifest_ts(manifest), os.path.join(path, 'manifest.ts'))
    print '\n--------------------------------------------------------------------'
...
if __name__ == '__main__':
    ...
    names = ["trials", "trials-weibo",  'test', 'abomination', "deathknight","sylvanas", "thief", "lich", "warden",'weapon-card', 'archer', 'box-game', 'climber']
